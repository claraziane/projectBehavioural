...
# Load result table
DATA <- read.csv("/Users/claraziane/Library/CloudStorage/OneDrive-UniversitedeMontreal/Projets/projetDT/Articles/articleBehavioural/SUBMITTED/PsyArXiv/dataTable_EXP2_IMI_CV.csv")
head(DATA)

DATA$Movement <- factor(DATA$Movement, levels = c("tapping", "walking")) 
DATA$Instruction <- factor(DATA$Instruction, levels = c("spontaneous", "synchronize"))
